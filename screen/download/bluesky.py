...
class BlueskyDownloadPage(QWidget):
    ...
    def initUI(self):
        ...
        top_bar = FunctionBar("download bluesky audio", font_family, self)
        layout.addLayout(top_bar)
        layout.addSpacing(10)

        # Tạo giao diện tải với placeholder tùy chỉnh
        self.download_ui = DownloadUI(placeholder_text="paste bluesky url")
        layout.addWidget(self.download_ui)

        # Kết nối nút download với hàm xử lý
        self.download_ui.download_btn.clicked.connect(self.download_bluesky_audio)
    ...
    def download_bluesky_audio(self):
        ...
        if not bluesky_link:
            print("Please enter a Bluesky link")
            return

        # Kiểm tra URL trước khi tải
        is_valid, message, platform = self.validator.is_valid_url(bluesky_link)
        if not is_valid or platform != 'bluesky':
            print(f"Error: {message if not is_valid else 'This is not a valid Bluesky URL'}")
            return

        # Nếu URL hợp lệ và là Bluesky, tiến hành tải
        name_label, size_label, progress_bar, main_layout = self.download_ui.add_download_item()
        self.worker = DownloadWorker(bluesky_link)
    ...
    def open_file_location(self):
        ...
        output_dir = os.path.join(documents_path, "audio-edita", "download", "bluesky")
