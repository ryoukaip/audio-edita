...
class BandcampDownloadPage(QWidget):
    def __init__(self):
        super().__init__()
    ...
    def initUI(self):
        ...
        top_bar = FunctionBar("download bandcamp audio", font_family, self)
        layout.addLayout(top_bar)
        layout.addSpacing(10)

        # Tạo giao diện tải với placeholder tùy chỉnh
        self.download_ui = DownloadUI(placeholder_text="paste bandcamp url")
        layout.addWidget(self.download_ui)

        # Kết nối nút download với hàm xử lý
        self.download_ui.download_btn.clicked.connect(self.download_bandcamp_audio)
    ...
    def download_bandcamp_audio(self):
        bandcamp_link = self.download_ui.link_input.text()
        if not bandcamp_link:
            print("Please enter a TikTok link")
            return

        # Kiểm tra URL trước khi tải
        is_valid, message, platform = self.validator.is_valid_url(bandcamp_link)
        if not is_valid or platform != 'bandcamp':
        ...
        self.worker = DownloadWorker(bandcamp_link)
    ...
    def open_file_location(self):
        ...
        output_dir = os.path.join(documents_path, "audio-edita", "download", "bandcamp")
