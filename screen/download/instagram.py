...
class InstagramDownloadPage(QWidget):
    ...
    def initUI(self):
        ...
        top_bar = FunctionBar("download instagram audio", font_family, self)
        layout.addLayout(top_bar)
        layout.addSpacing(10)

        # Tạo giao diện tải với placeholder tùy chỉnh
        self.download_ui = DownloadUI(placeholder_text="paste instagram url")
        layout.addWidget(self.download_ui)

        # Kết nối nút download với hàm xử lý
        self.download_ui.download_btn.clicked.connect(self.download_instagram_audio)
    ...
    def download_instagram_audio(self):
        ...
        is_valid, message, platform = self.validator.is_valid_url(instagram_link)
        if not is_valid or platform != 'instagram':
        ...
        self.worker = DownloadWorker(instagram_link)
    ...
    def open_file_location(self):
        ...
        output_dir = os.path.join(documents_path, "audio-edita", "download", "instagram")
