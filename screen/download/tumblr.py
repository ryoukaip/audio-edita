...
class TumblrDownloadPage(QWidget):
    def __init__(self):
        super().__init__()
    ...
    def initUI(self):
        ...
        top_bar = FunctionBar("download tumblr audio", font_family, self)
        layout.addLayout(top_bar)
        layout.addSpacing(10)

        # Tạo giao diện tải với placeholder tùy chỉnh
        self.download_ui = DownloadUI(placeholder_text="paste tumblr url")
        layout.addWidget(self.download_ui)

        # Kết nối nút download với hàm xử lý
        self.download_ui.download_btn.clicked.connect(self.download_tumblr_audio)
    ...
    def download_tumblr_audio(self):
        tumblr_link = self.download_ui.link_input.text()
        if not tumblr_link:
            print("Please enter a tumblr link")
            return

        # Kiểm tra URL trước khi tải
        is_valid, message, platform = self.validator.is_valid_url(tumblr_link)
        if not is_valid or platform != 'tumblr':
        ...
        self.worker = DownloadWorker(tumblr_link)
    ...
    def open_file_location(self):
        ...
        output_dir = os.path.join(documents_path, "audio-edita", "download", "tumblr")
