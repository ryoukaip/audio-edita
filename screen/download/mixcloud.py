...
class MixcloudDownloadPage(QWidget):
    def __init__(self):
        super().__init__()
    ...
    def initUI(self):
        ...
        top_bar = FunctionBar("download mixcloud audio", font_family, self)
        layout.addLayout(top_bar)
        layout.addSpacing(10)

        # Tạo giao diện tải với placeholder tùy chỉnh
        self.download_ui = DownloadUI(placeholder_text="paste mixcloud url")
        layout.addWidget(self.download_ui)

        # Kết nối nút download với hàm xử lý
        self.download_ui.download_btn.clicked.connect(self.download_mixcloud_audio)
    ...
    def download_mixcloud_audio(self):
        mixcloud_link = self.download_ui.link_input.text()
        if not mixcloud_link:
            print("Please enter a TikTok link")
            return

        # Kiểm tra URL trước khi tải
        is_valid, message, platform = self.validator.is_valid_url(mixcloud_link)
        if not is_valid or platform != 'mixcloud':
        ...
        self.worker = DownloadWorker(mixcloud_link)
    ...
    def open_file_location(self):
        ...
        output_dir = os.path.join(documents_path, "audio-edita", "download", "mixcloud")
