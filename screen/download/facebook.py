...
class FacebookDownloadPage(QWidget):
    def __init__(self):
        super().__init__()
    ...
    def initUI(self):
        ...
        top_bar = FunctionBar("download facebook audio", font_family, self)
        layout.addLayout(top_bar)
        layout.addSpacing(10)

        # Tạo giao diện tải với placeholder tùy chỉnh
        self.download_ui = DownloadUI(placeholder_text="facebook.com/moritaka.sofujinia")
        layout.addWidget(self.download_ui)

        # Kết nối nút download với hàm xử lý
        self.download_ui.download_btn.clicked.connect(self.download_facebook_audio)
    ...
    def download_facebook_audio(self):
        facebook_link = self.download_ui.link_input.text()
        if not facebook_link:
            print("Please enter a TikTok link")
            return

        # Kiểm tra URL trước khi tải
        is_valid, message, platform = self.validator.is_valid_url(facebook_link)
        if not is_valid or platform != 'facebook':
        ...
        self.worker = DownloadWorker(facebook_link)
    ...
    def open_file_location(self):
        ...
        output_dir = os.path.join(documents_path, "audio-edita", "download", "facebook")
