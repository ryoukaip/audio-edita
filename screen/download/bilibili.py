...
class BilibiliDownloadPage(QWidget):
    def __init__(self):
        super().__init__()
    ...
    def initUI(self):
        ...
        top_bar = FunctionBar("download bilibili audio", font_family, self)
        layout.addLayout(top_bar)
        layout.addSpacing(10)

        # Tạo giao diện tải với placeholder tùy chỉnh
        self.download_ui = DownloadUI(placeholder_text="paste bilibili url")
        layout.addWidget(self.download_ui)

        # Kết nối nút download với hàm xử lý
        self.download_ui.download_btn.clicked.connect(self.download_bilibili_audio)
    ...
    def download_bilibili_audio(self):
        bilibili_button = self.download_ui.link_input.text()
        if not bilibili_button:
            print("Please enter a bilibili link")
            return

        # Kiểm tra URL trước khi tải
        is_valid, message, platform = self.validator.is_valid_url(bilibili_button)
        if not is_valid or platform != 'bilibili':
            print(f"Error: {message if not is_valid else 'This is not a valid bilibili URL'}")
            return
        ...
        self.worker = DownloadWorker(bilibili_button)
    ...
    def open_file_location(self):
        ...
        output_dir = os.path.join(documents_path, "audio-edita", "download", "bilibili")
